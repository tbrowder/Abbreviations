...
C<:output-type> options: 
C<AH> (AbbrevHash), C<AL> (AbbrevList), C<H> (Hash), C<L> (List), or C<S> (Str). 
For example,
...
The default B<Hash> (C<H>) returned will
...
An B<AbbrevHash> (C<AH>) is keyed by all of the valid abbreviations for the input 
word list and whose values are the word from which that abbreviation is defined.

=item AbbrevList

An B<AbbrevList> (C<AL>) is special in that the returned list is the one, shortest
...
A B<List> (C<L>) contains all of the valid abbreviations for the input 
word list, including the words themselves, sorted first be the default Raku sort
and then by length (shortest first).

=item String

A B<String> (C<S>) is the string formed by joining the B<List> by a single space.

One other point about the new abbreviation process: the input word set is
first formed into subgroups based on the the first
character of each word. Then the subgroups have
...
The routine sorts the input list first by the default Raku sort and then
