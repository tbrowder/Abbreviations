...
quietly eliminated. An empty word set will cause an exception.
...
A I<String> (C<S>) is the string formed by joining the I<List> by a single space.
...
since we no longer have any subgroups so the I<min-abbrev-len> is again 
three. 
...
=head3 C<enum Other-type>

The I<enum> C<Other-type> is exported automatically as it is required for use of C<sub abbreviations>.
...
This the routine, slightly modified, taken from the I<Rosetta Code> website. 
