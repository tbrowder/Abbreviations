...
formed into subgroups comprised of each imput word. Abbreviations
...
the I<min-abbrev-len> is one or two for each word and
...
since we have fewer unique words so there are fewer unique abbreviations.
